import json
from pathlib import Path


ROOT_DIR = Path(__file__).resolve().parents[2]

ARCHIVO_DATOS = (
    ROOT_DIR
    / "data"
    / "raw"
    / "estaciones_troncales_raw.json"
)


def main():

    if not ARCHIVO_DATOS.exists():
        print("❌ No se encontró el archivo:")
        print(ARCHIVO_DATOS)
        return

    with open(ARCHIVO_DATOS, "r", encoding="utf-8") as archivo:
        datos = json.load(archivo)

    print("\n===================================")
    print("    INSPECCIÓN DE ESTACIONES")
    print("===================================\n")

    print("Archivo:")
    print(ARCHIVO_DATOS)

    print("\nTipo de dato:")
    print(type(datos).__name__)

    if "features" not in datos:
        print("\n❌ No se encontró 'features'.")
        return

    estaciones = datos["features"]

    print("\nCantidad de registros:")
    print(len(estaciones))

    if not estaciones:
        print("\n❌ No hay estaciones.")
        return

    primera = estaciones[0]

    print("\nEstructura del primer registro:")
    print("-----------------------------------")

    print("\nClaves:")

    for clave in primera.keys():
        print(f"  - {clave}")

    print("\nAtributos:")

    atributos = primera.get("attributes", {})

    for clave, valor in atributos.items():
        print(f"  {clave}: {valor}")

    print("\nGeometría:")

    geometria = primera.get("geometry")

    if geometria:
        for clave, valor in geometria.items():
            print(f"  {clave}: {valor}")
    else:
        print("  No tiene geometría.")


if __name__ == "__main__":
    main()